...
def parse_args():
    ...
    parser.add_argument("--train_embed_file_paths", type=str, required=True, help=";-separated paths to the train embed files.")
    parser.add_argument("--dev_embed_file_paths", type=str, required=True, help=";-separated paths to the dev embed files.")
    parser.add_argument("--total_train_split_num", type=int, required=True, help="number of train splits.")
...
def _generate_epoch_dataloader(train_dataset_config, dev_dataset_config, total_train_split_num):
    """
    Generates a training data loader for a specific epoch by selecting a random split
    of the dataset. The dataset is divided into multiple splits, and one split is loaded
    per epoch to manage memory and storage efficiently.

    This function updates the dataset configuration paths for the selected split and
    initializes a new training data loader with these paths. It also returns a fixed
    development data loader, which remains constant throughout the training process.

    Args:
        train_dataset_config (DatasetConfig): Configuration object containing paths and
            settings for the training dataset.
        dev_dataset_config (DatasetConfig): Configuration object containing paths and
            settings for the development dataset.
        total_train_split_num (int): Total number of splits available in the dataset.

    Returns:
        Tuple[DataLoader, DataLoader]: A tuple containing the newly created training
            data loader for the current epoch and the fixed development data loader.
    """
    # Randomly choose a training split index for this epoch
    train_split_index = random.randint(0, total_train_split_num-1)

    # Create new configurations with updated paths
    updated_train_config = DatasetConfig(
        token_file_path=train_dataset_config.token_file_path.replace('SPLIT', str(train_split_index)),
        embed_file_paths=[path.replace('SPLIT', str(train_split_index)) for path in train_dataset_config.embed_file_paths],
        word_seg_file_path=train_dataset_config.word_seg_file_path,
        batch_size=train_dataset_config.batch_size,
        max_seq_length=train_dataset_config.max_seq_length,
        shuffle=train_dataset_config.shuffle,
        num_workers=train_dataset_config.num_workers
    )

    # Update train dataloaders with updated train config
    train_dataloader = get_train_loader(updated_train_config)
   
    # Dev dataloaders are fixed and do not need a new config
    dev_dataloader = get_eval_loader(dev_dataset_config)

    return train_dataloader, dev_dataloader

def train(model, train_dataset_config: DatasetConfig, dev_dataset_config: DatasetConfig, total_train_split_num, current_epoch, epochs, logger, tb_writer, log_interval, peak_lr, gradient_acc_steps, save_dir, device):
    dummy_train_dataloader, _ = _generate_epoch_dataloader(train_dataset_config, dev_dataset_config, total_train_split_num)
    total_steps = epochs * len(dummy_train_dataloader)
    current_step = current_epoch * len(dummy_train_dataloader)
    best_loss, best_acc = float('inf'), 0.0
    no_improve_epochs = 0
    patience = 5  # Number of epochs to wait before early stopping

    for epoch in range(current_epoch, epochs):
        def _train(current_step, train_dataloader):
            model.train()
        ...
        curr_epoch_train_dataloader, curr_epoch_dev_dataloader = _generate_epoch_dataloader(train_dataset_config, dev_dataset_config, total_train_split_num)
        train_loss, current_step = _train(current_step, curr_epoch_train_dataloader)
        val_loss, val_acc = _val(curr_epoch_dev_dataloader)
        ...
        # Checkpoint for best accuracy, if needed
        if val_acc > best_acc:
            best_acc = val_acc
...
def main():
    ...
    train_dataset_config = DatasetConfig(
        ...
        embed_file_paths=train_embed_file_paths,
        word_seg_file_path=args.word_seg_file_path,
    ...
    dev_dataset_config = DatasetConfig(
        ...
        embed_file_paths=dev_embed_file_paths,
        word_seg_file_path=args.word_seg_file_path,
        batch_size=args.batch_size,
        max_seq_length=args.max_seq_length,
        shuffle=False,
    ...
    # Run the training
    train(model, train_dataset_config, dev_dataset_config, args.total_train_split_num, current_epoch, args.epochs, logger, tb_writer, args.log_interval, args.learning_rate, args.gradient_acc_steps, args.save_dir, device)
    tb_writer.close()
