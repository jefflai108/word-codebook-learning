...
def greedy_decode(model, num_classes, inference_dataloader, max_seq_length, logger, device): 
    ...
    model.eval()
    with torch.no_grad():
        for uttid, X, src_mask, src_key_padding_mask, num_segments in inference_dataloader:
            assert len(uttid) == 1
            ...
            decoded_X = model.greedy_decode_step(X, src_mask, src_key_padding_mask, max_seq_length)
...
def main():
    ...
    # Setup dataloaders 
    train_data_loader = get_inference_loader(args.train_token_file_path, args.train_embed_file_path, args.word_seg_file_path, args.segment_context_size, batch_size=1, shuffle=False, num_workers=1, max_seq_len=args.max_seq_length)
    test_data_loader = get_inference_loader(args.test_token_file_path, args.test_embed_file_path, args.word_seg_file_path, args.segment_context_size, batch_size=1, shuffle=False, num_workers=1, max_seq_len=args.max_seq_length)
    dev_data_loader = get_inference_loader(args.dev_token_file_path, args.dev_embed_file_path, args.word_seg_file_path, args.segment_context_size, batch_size=1, shuffle=False, num_workers=1, max_seq_len=args.max_seq_length)
