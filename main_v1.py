...
def train(model, train_dataloader, dev_dataloader, current_epoch, epochs, logger, tb_writer, log_interval, peak_lr, save_dir, device):
    ...
    for epoch in range(current_epoch, epochs):
        def _train(current_step):
            ...
            for batch_idx, (X, Y, src_mask, tgt_mask, src_key_padding_mask, tgt_key_padding_mask, memory_key_padding_mask) in enumerate(train_dataloader):
                current_step += 1
        ...
        def _val():
            ...
            total_loss, total_acc = 0, 0
            with torch.no_grad():
                for X, Y, src_mask, tgt_mask, src_key_padding_mask, tgt_key_padding_mask, memory_key_padding_mask in dev_dataloader:
                    # Move tensors to the right device
                    X, Y = X.to(device), Y.to(device)
                    tgt_mask = tgt_mask.to(device)
                    src_key_padding_mask, tgt_key_padding_mask, memory_key_padding_mask = src_key_padding_mask.to(device), tgt_key_padding_mask.to(device), memory_key_padding_mask.to(device)
...
def main():
    ...
    # Setup dataloaders 
    train_data_loader = get_train_loader(args.train_token_file_path, args.train_embed_file_path, args.word_seg_file_path, args.segment_context_size, args.batch_size, shuffle=True, num_workers=1, max_seq_len=args.max_seq_length)
    dev_data_loader = get_eval_loader(args.dev_token_file_path, args.dev_embed_file_path, args.word_seg_file_path, args.segment_context_size, args.batch_size, shuffle=False, num_workers=1, max_seq_len=args.max_seq_length)
