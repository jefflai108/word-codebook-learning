...
class TestSpeechTransformerModel(unittest.TestCase):
    def setUp(self):
        ...
        self.model = SpeechTransformerModel(
            ...
            num_layer_repre=self.num_layer_repre,
    ...
    #    # Create dummy data
    #    #X_flattened : (64, 48, 1024)  
    #    #Y_flattened : (64, 24) 
    #    #src_mask : None 
    #    #tgt_mask: (24, 24) 
    #    #src_key_padding_mask: (64, 48) 
    #    #tgt_key_padding_mask: (64, 24)
    #    #memory_key_padding_mask: (64, 48) 
    #    
    #    src = torch.randint(0, self.vocab_size, (self.batch_size, self.src_seq_len, self.input_dim * self.num_layer_repre)).float()
    #    tgt = torch.randint(0, self.vocab_size, (self.batch_size, self.tgt_seq_len))
    #    src_mask = None 
    ...
    #    tgt_key_padding_mask = torch.zeros(self.batch_size, self.tgt_seq_len).type(torch.bool)
    ...
    #    # Check output shape
    #    self.assertEqual(output.shape, (self.tgt_seq_len, self.batch_size, self.vocab_size))
    ...
    #    initial_loss = self.model.train_step(src, tgt_input, tgt, src_mask, tgt_mask, src_key_padding_mask, tgt_key_padding_mask, memory_key_padding_mask, 0, 4)
    #    for i in range(1, 100):  # Train for a few steps
    #        loss, (acc, _, _) = self.model.train_step(src, tgt_input, tgt, src_mask, tgt_mask, src_key_padding_mask, tgt_key_padding_mask, memory_key_padding_mask, i, 4)
