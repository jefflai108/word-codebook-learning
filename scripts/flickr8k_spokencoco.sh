...
if [ $stage -eq 0 ]; then 
    ...
    CUDA_LAUNCH_BLOCKING=1 python main_v2_1_flickr8k_spokencoco.py \
               ...
               --save_dir ${expdir} \
               --batch_size 16 --gradient_acc_steps 16 \
               --repre_dim ${NUM_PCA} \
               --num_layer_repre 5 \
               --word_pooling "lde32" \
               --vocab_size 1027 --d_model 768 --nhead 8 --num_encoder_layers 6 --num_decoder_layers 6 --dim_feedforward 3072 --model_activation "gelu" \
               --dropout 0.1 --max_seq_length 100 --epochs 50 --log_interval 20 --learning_rate 1e-4 --optimizer_type "adam" --label_smoothing 0.1
fi 
